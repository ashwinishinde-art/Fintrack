...
<head>
    ...
    <style>
        ...
        /* ============================================================
           LOGIN SCREEN — extra micro-interactions & cursor-reactive fx
           The blobs keep their own infinite float animation (above);
           a separate wrapper div is what JS nudges on mousemove, so the
           two transforms never fight over the same element.
           ============================================================ */
        .login-blob-parallax { position: absolute; inset: 0; pointer-events: none; transition: transform .3s cubic-bezier(.16,1,.3,1); will-change: transform; }

        /* Card: subtle 3D tilt + a soft spotlight glow that follows the cursor */
        #view-login .login-card {
            transition: transform .15s ease-out, box-shadow .3s ease, border-color .3s ease;
            transform-style: preserve-3d;
            will-change: transform;
        }
        #view-login .login-card::before {
            content: '';
            position: absolute;
            inset: 0;
            border-radius: inherit;
            background: radial-gradient(280px circle at var(--spot-x, 50%) var(--spot-y, 0%), rgba(0,196,140,.14), transparent 70%);
            opacity: 0;
            transition: opacity .35s ease;
            pointer-events: none;
        }
        #view-login .login-card.spotlight-active::before { opacity: 1; }
        #view-login .login-card.spotlight-active { border-color: #2A3A56; box-shadow: 0 25px 50px -20px rgba(0,0,0,.5), 0 0 0 1px rgba(0,196,140,.08); }

        /* Logo mark: playful pop + tilt on hover */
        .login-logo-mark { transition: transform .35s cubic-bezier(.34,1.56,.64,1), box-shadow .35s ease; }
        .login-logo-mark:hover { transform: scale(1.08) rotate(-6deg); box-shadow: 0 0 0 6px rgba(0,196,140,.15), 0 14px 26px -10px rgba(0,196,140,.55); }

        /* Auth tab buttons: gentle lift on hover (bg swap is still handled by JS) */
        #btn-toggle-signin:hover, #btn-toggle-signup:hover { transform: translateY(-1px); }

        /* Password show/hide icon: pop on hover instead of just a color fade */
        .eye-toggle-btn i { transition: transform .2s cubic-bezier(.34,1.56,.64,1); display: inline-block; }
        .eye-toggle-btn:hover i { transform: scale(1.18); }

        /* Text inputs: soft lift on hover, ahead of the focus state */
        #view-login input:not(:focus):hover {
            border-color: #2A3A56;
            box-shadow: 0 0 0 3px rgba(0,196,140,.07);
        }

        /* Sign-in / register submit buttons: light diagonal shine sweep on hover */
        #signin-form button[type="submit"], #signup-form button[type="submit"] {
            position: relative;
            overflow: hidden;
        }
        #signin-form button[type="submit"]::after, #signup-form button[type="submit"]::after {
            content: '';
            position: absolute;
            top: 0; left: -60%;
            width: 40%; height: 100%;
        ...
        #signin-form button[type="submit"]:hover::after, #signup-form button[type="submit"]:hover::after { left: 130%; }

        @media (prefers-reduced-motion: reduce) {
            #view-login .login-card, .login-blob-parallax, .login-logo-mark { transition: none !important; transform: none !important; }
        }
...
<body class="text-slate-800 min-h-screen flex flex-col">
    ...
    <div id="view-login" class="flex-1 flex flex-col justify-center items-center p-4 bg-[#0B1220] relative overflow-hidden">
        <div class="login-blob-parallax login-blob-parallax-a"><div class="login-blob login-blob-a"></div></div>
        <div class="login-blob-parallax login-blob-parallax-b"><div class="login-blob login-blob-b"></div></div>
        <div class="login-card max-w-md w-full bg-[#0E1626] border border-slate-800 p-8 rounded-2xl shadow-2xl space-y-6 animate-in relative z-10">
            <div class="text-center space-y-2">
                <div class="flex flex-col items-center justify-center gap-3 mb-2">
                    <div class="login-logo-mark h-16 w-16 bg-[#00C48C] rounded-2xl flex items-center justify-center text-white text-3xl font-black shadow-lg shadow-emerald-500/20">
            ...
            <form id="signin-form" onsubmit="handleLogin(event)" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <button type="button" onclick="togglePasswordVisibility('login-password', 'login-eye-icon')" class="eye-toggle-btn absolute inset-y-0 right-3 flex items-center px-2 text-slate-400 hover:text-white transition-all">
            ...
            <form id="signup-form" onsubmit="handleRegister(event)" class="hidden space-y-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <button type="button" onclick="togglePasswordVisibility('reg-password', 'reg-eye-icon')" class="eye-toggle-btn absolute inset-y-0 right-3 flex items-center px-2 text-slate-400 hover:text-white transition-all">
