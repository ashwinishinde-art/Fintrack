...
<head>
    ...
    <script>
        // Class-based dark mode: toggling `dark` on <html> (done by the
        // theme switch in Profile Settings) drives every dark: utility
        // below, plus the hand-written html.dark overrides further down.
        tailwind.config = { darkMode: 'class' };
    </script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/Chart.js/4.4.4/chart.umd.min.js"></script>
    <script>
        // Some networks/browsers/extensions block cdnjs.cloudflare.com specifically.
        // If the primary CDN above failed to load Chart.js, fall back to a
        // second CDN (jsDelivr) before the page continues, so the app still
        // works instead of leaving the Analytics chart permanently blank.
        if (typeof Chart === 'undefined') {
            document.write('<script src="https:\/\/cdn.jsdelivr.net\/npm\/chart.js@4.4.4\/dist\/chart.umd.min.js"><\/script>');
        }
    </script>
    <script>
        // If BOTH CDNs failed (e.g. no internet access at all), fall back once more.
        if (typeof Chart === 'undefined') {
    ...
    <style>
        ...
        .sidebar-item-active { background-color: #1E293B; border-left: 4px solid #00C48C; color: #ffffff; animation: sidebarActivate .35s ease; }
        /* Some browsers (e.g. Edge) render their own built-in "reveal
           password" eye icon inside password fields, which shows up
           stacked on top of our custom eye toggle button. Hide the
           native one so only our own icon appears. */
        ...
        #view-app aside::-webkit-scrollbar-thumb { background: #334155; background-clip: padding-box; }

        /* Visible keyboard focus everywhere */
        button:focus-visible, a:focus-visible, input:focus-visible, select:focus-visible, textarea:focus-visible {
            outline: 2px solid #00C48C;
            outline-offset: 2px;
            border-radius: 4px;
        }

        /* Every white card in the app shares this exact class combination,
           so one rule gives the whole app a consistent hover-lift without
           having to touch every card individually. */
        .rounded-2xl.border-slate-100.shadow-sm {
            transition: transform .25s cubic-bezier(.16,1,.3,1), box-shadow .25s ease, border-color .25s ease, background-color .35s ease;
        }
        .rounded-2xl.border-slate-100.shadow-sm:hover {
            transform: translateY(-3px);
            box-shadow: 0 20px 35px -20px rgba(15, 23, 42, .22);
            border-color: #E2E8F0;
        }

        /* Sidebar nav micro-interactions */
        #view-app aside nav > button { transition: background-color .2s ease, color .2s ease, transform .2s ease; }
        #view-app aside nav > button:hover:not(.sidebar-item-active) { transform: translateX(3px); }
        #view-app aside nav > button i { transition: transform .25s cubic-bezier(.34,1.56,.64,1); display: inline-block; }
        #view-app aside nav > button:hover i { transform: scale(1.15); }

        /* Press feedback for buttons across the app */
        #view-app button, #view-login button, #budget-modal button, #budget-exceeded-modal button {
            transition: transform .12s ease, background-color .2s ease, box-shadow .2s ease, color .2s ease, border-color .2s ease;
        }
        #view-app main button:active, #view-login button:active, #view-app header button:active,
        #budget-modal button:active, #budget-exceeded-modal button:active {
        ...
        @keyframes sidebarActivate { from { background-color: transparent; } to { background-color: #1E293B; } }
        ...
        .animate-in { animation: fadeInUp .5s cubic-bezier(.16,1,.3,1) both; }
        .animate-fade-row { animation: fadeInRow .4s ease both; }
        .modal-panel-pop { animation: modalPop .28s cubic-bezier(.34,1.56,.64,1) both; }
        .modal-backdrop-fade { animation: backdropFade .2s ease both; }
        ...
        /* Shake + red flash for failed form submissions (login/register) */
        @keyframes shakeX {
        ...
        .animate-shake { animation: shakeX .5s cubic-bezier(.36,.07,.19,.97) both; }
        .field-error { border-color: #F43F5E !important; }

        /* Tab-switch cross-fade */
        @keyframes tabFadeIn { from { opacity: 0; transform: translateY(8px); } to { opacity: 1; transform: translateY(0); } }
        .tab-fade-in { animation: tabFadeIn .35s cubic-bezier(.16,1,.3,1) both; }

        /* Spinner used inside buttons during async submits */
        @keyframes spin { to { transform: rotate(360deg); } }
        .btn-spinner { display: inline-block; width: .85em; height: .85em; border: 2px solid currentColor; border-right-color: transparent; border-radius: 50%; animation: spin .6s linear infinite; }

        /* Ambient drifting glow behind the login card */
        @keyframes floatBlobA { 0%,100% { transform: translate(0,0) scale(1); } 50% { transform: translate(24px,-18px) scale(1.08); } }
        @keyframes floatBlobB { 0%,100% { transform: translate(0,0) scale(1); } 50% { transform: translate(-20px,20px) scale(1.06); } }
        .login-blob { position: absolute; border-radius: 9999px; filter: blur(70px); opacity: .35; pointer-events: none; }
        .login-blob-a { width: 320px; height: 320px; background: #00C48C; top: -80px; left: -80px; animation: floatBlobA 9s ease-in-out infinite; }
        .login-blob-b { width: 280px; height: 280px; background: #2563EB; bottom: -70px; right: -60px; animation: floatBlobB 11s ease-in-out infinite; }

        /* Subtle pulse to draw attention to a newly-triggered overspend banner */
        @keyframes attentionPulse { 0%,100% { box-shadow: 0 0 0 0 rgba(168,57,37,0); } 50% { box-shadow: 0 0 0 6px rgba(168,57,37,.08); } }
        .animate-attention { animation: attentionPulse 1.8s ease-in-out 2; }

        /* ============================================================
           MORE HOVER MICRO-INTERACTIONS
           ============================================================ */
        /* Icon badges inside any card that already has the hover-lift —
           the icon itself gets a little extra life on top of the lift. */
        .rounded-2xl.border-slate-100.shadow-sm:hover .h-8.w-8.rounded-xl {
            transform: scale(1.14) rotate(-6deg);
        }
        .h-8.w-8.rounded-xl { transition: transform .3s cubic-bezier(.34,1.56,.64,1); }

        /* The four investment-tip cards inside the Quotes panel */
        .tip-card { transition: transform .2s ease, border-color .2s ease, background-color .2s ease; }
        .tip-card:hover { transform: translateY(-2px) scale(1.015); border-color: #00C48C; background-color: #F0FDF9; }
        html.dark .tip-card:hover { background-color: #10261F; }

        /* Rotating quote card: pause the progress bar and lift gently on hover */
        .quote-card { transition: transform .3s cubic-bezier(.16,1,.3,1); }
        .quote-card:hover { transform: translateY(-2px) scale(1.008); }
        .quote-card:hover #quote-rotator-progress { animation-play-state: paused; }

        /* Table rows: left accent bar slides in + gentle indent on hover */
        #transactions-table-body tr { position: relative; transition: transform .18s ease, background-color .15s ease; }
        #transactions-table-body tr:hover { transform: translateX(2px); }
        #transactions-table-body tr::before {
            content: ''; position: absolute; left: -1px; top: 0; bottom: 0; width: 3px;
            background: #00C48C; transform: scaleY(0); transition: transform .18s ease; transform-origin: center;
        }
        #transactions-table-body tr:hover::before { transform: scaleY(1); }

        /* Delete-expense icon wiggles instead of just recoloring */
        #transactions-table-body button[aria-label="Delete this expense"]:hover i { animation: wiggle .35s ease; }
        @keyframes wiggle { 0%,100% { transform: rotate(0); } 25% { transform: rotate(-12deg); } 75% { transform: rotate(12deg); } }

        /* Buttons with a leading/trailing icon get a tiny icon nudge on hover */
        button[type="submit"]:hover i:first-child,
        #save-budgets-btn:hover i,
        button[onclick="advanceQuoteRotator()"]:hover i {
            transform: translateX(2px);
        }
        button[type="submit"] i:first-child, #save-budgets-btn i, button[onclick="advanceQuoteRotator()"] i {
            display: inline-block; transition: transform .2s ease;
        }

        /* Category pie-chart cards get a bit of extra scale beyond the shared lift */
        #category-progress-list > div:hover svg { transform: scale(1.06); }
        #category-progress-list svg { transition: transform .3s cubic-bezier(.34,1.56,.64,1); }

        /* Login inputs glow softly on hover, before focus even lands */
        #view-login input { transition: border-color .2s ease, background-color .2s ease, box-shadow .2s ease; }
        #view-login input:hover:not(:focus) { border-color: #334155; box-shadow: 0 0 0 3px rgba(0,196,140,.08); }

        /* Cursor-tracking spotlight + blob parallax on the login screen */
        .login-blob-wrap { position: absolute; inset: 0; transition: transform .25s cubic-bezier(.16,1,.3,1); will-change: transform; }
        .login-spotlight {
            position: absolute; inset: 0; pointer-events: none;
            background: radial-gradient(500px circle at var(--mx, 50%) var(--my, 50%), rgba(0,196,140,.10), transparent 55%);
            transition: opacity .3s ease;
        }
        @media (prefers-reduced-motion: reduce) {
            .login-blob-wrap { transition: none; }
            .login-spotlight { display: none; }
        }

        /* ============================================================
           DARK MODE
           Scoped to the main app content (#view-app > main) and the two
           modals — the sidebar and the login screen are already dark by
           design and stay exactly as they are in both themes.
           ============================================================ */
        html.dark body { background-color: #0B1220; }

        html.dark #view-app > main,
        html.dark #budget-modal,
        html.dark #budget-exceeded-modal { color-scheme: dark; }

        /* Surfaces */
        html.dark #view-app > main .bg-white,
        html.dark #budget-modal .bg-white,
        html.dark #budget-exceeded-modal .bg-white { background-color: #121B2E; }

        html.dark #view-app > main .bg-slate-50,
        html.dark #budget-modal .bg-slate-50,
        html.dark #view-app > main input.bg-slate-50,
        html.dark #view-app > main select.bg-slate-50 { background-color: #0D1526; }
        html.dark #view-app > main .bg-slate-50\/50 { background-color: rgba(13,21,38,.6); }
        html.dark #view-app > main .bg-slate-100,
        html.dark #budget-modal .bg-slate-100 { background-color: #1C283D; }
        html.dark #view-app > main .bg-slate-100\/30 { background-color: rgba(28,40,61,.5); }
        html.dark #view-app > main .hover\:bg-slate-50:hover { background-color: #17233A; }
        html.dark #view-app > main .hover\:bg-slate-200:hover { background-color: #2A3A56; }
        html.dark #view-app > main .focus\:bg-white:focus { background-color: #121B2E; }

        /* Borders & dividers */
        html.dark #view-app > main .border-slate-100,
        html.dark #budget-modal .border-slate-100,
        html.dark #budget-exceeded-modal .border-slate-100 { border-color: #223050; }
        html.dark #view-app > main .border-slate-200,
        html.dark #budget-modal .border-slate-200 { border-color: #2A3A56; }
        html.dark #view-app > main .divide-slate-100 > :not([hidden]) ~ :not([hidden]) { border-color: #223050; }

        /* Text */
        html.dark #view-app > main .text-slate-900,
        html.dark #budget-modal .text-slate-900 { color: #F1F5F9; }
        html.dark #view-app > main .text-slate-800,
        html.dark #budget-modal .text-slate-800,
        html.dark #budget-exceeded-modal .text-slate-800 { color: #E2E8F0; }
        html.dark #view-app > main .text-slate-700 { color: #B8C4D9; }
        html.dark #view-app > main .text-slate-600,
        html.dark #budget-modal .text-slate-600 { color: #94A3B8; }
        html.dark #view-app > main .text-slate-500,
        html.dark #budget-modal .text-slate-500,
        html.dark #budget-exceeded-modal .text-slate-500 { color: #8695AC; }
        html.dark #view-app > main .text-slate-400 { color: #64748B; }

        /* Header + table head shading */
        html.dark #view-app > main header.bg-white { background-color: #0E1626; }

        /* Soft accent badges (category pills, KPI icon chips, info boxes) */
        html.dark #view-app > main .bg-emerald-50 { background-color: rgba(16,185,129,.12); }
        html.dark #view-app > main .bg-indigo-50,
        html.dark #budget-modal .bg-indigo-50 { background-color: rgba(99,102,241,.14); }
        html.dark #budget-modal .bg-indigo-50\/50 { background-color: rgba(99,102,241,.08); }
        html.dark #budget-modal .bg-indigo-100 { background-color: rgba(99,102,241,.22); }
        html.dark #view-app > main .bg-amber-50 { background-color: rgba(245,158,11,.14); }
        html.dark #view-app > main .bg-cyan-50 { background-color: rgba(34,211,238,.14); }
        html.dark #view-app > main .bg-purple-50 { background-color: rgba(168,85,247,.14); }
        html.dark #view-app > main .bg-rose-50,
        html.dark #budget-exceeded-modal .bg-rose-50 { background-color: rgba(244,63,94,.14); }
        html.dark #view-app > main .bg-rose-50\/50 { background-color: rgba(244,63,94,.08); }
        html.dark #view-app > main .hover\:bg-rose-50:hover { background-color: rgba(244,63,94,.16); }
        html.dark #view-app > main .hover\:border-rose-300:hover { border-color: rgba(244,63,94,.5); }
        html.dark #view-app > main .border-emerald-100 { border-color: rgba(16,185,129,.3); }
        html.dark #budget-modal .border-indigo-100\/80 { border-color: rgba(99,102,241,.3); }
        html.dark #budget-modal .border-indigo-200 { border-color: rgba(99,102,241,.4); }
        html.dark #view-app > main .border-amber-100 { border-color: rgba(245,158,11,.3); }
        html.dark #view-app > main .border-cyan-100 { border-color: rgba(34,211,238,.3); }
        html.dark #view-app > main .border-purple-100 { border-color: rgba(168,85,247,.3); }
        html.dark #view-app > main .border-rose-100 { border-color: rgba(244,63,94,.3); }
        html.dark #view-app > main .border-rose-200 { border-color: rgba(244,63,94,.35); }
        html.dark #view-app > main .text-amber-700 { color: #FCD34D; }
        html.dark #view-app > main .text-cyan-700 { color: #67E8F9; }
        html.dark #view-app > main .text-purple-700 { color: #D8B4FE; }
        html.dark #view-app > main .text-rose-700 { color: #FDA4AF; }
        html.dark #budget-modal .text-indigo-700 { color: #A5B4FC; }
        html.dark #budget-modal .text-indigo-800 { color: #C7D2FE; }
        html.dark #budget-modal .text-indigo-950 { color: #E0E7FF; }

        /* Overspend banner */
        html.dark #view-app > main .bg-\[\#FFF5F3\] { background-color: rgba(168,57,37,.14); }
        html.dark #view-app > main .border-\[\#FDE2DC\] { border-color: rgba(168,57,37,.35); }

        /* Scrollbar inside the main content area */
        html.dark #view-app > main::-webkit-scrollbar-thumb { background: #2A3A56; background-clip: padding-box; }
        html.dark #view-app > main::-webkit-scrollbar-thumb:hover { background: #3B4D6E; background-clip: padding-box; }

        /* ============================================================
           THEME TOGGLE SWITCH (Profile → Appearance)
           ============================================================ */
        #theme-toggle-switch { transition: background-color .3s ease; }
        #theme-toggle-thumb { transition: transform .4s cubic-bezier(.34,1.56,.64,1); }
        @keyframes iconPop { 0% { transform: scale(.4) rotate(-100deg); opacity: 0; } 100% { transform: scale(1) rotate(0deg); opacity: 1; } }
        .theme-icon-pop { animation: iconPop .45s cubic-bezier(.34,1.56,.64,1) both; }

        /* ============================================================
           CIRCULAR REVEAL — the expanding-circle wipe used when the
           theme is switched (Chromium browsers via the View Transitions
           API). Everywhere else just cross-fades via the transitions
           declared above; both respect prefers-reduced-motion already.
           ============================================================ */
        ...
        /* Playful trash-icon wiggle in the transactions table */
        #transactions-table-body button i { transition: transform .2s cubic-bezier(.34,1.56,.64,1); display: inline-block; }
        #transactions-table-body button:hover i { transform: scale(1.18) rotate(-10deg); }

        /* Brief green flash to confirm a save landed (e.g. new expense recorded) */
        @keyframes successFlash { 0% { box-shadow: 0 0 0 0 rgba(0,196,140,0); } 30% { box-shadow: 0 0 0 6px rgba(0,196,140,.18); } 100% { box-shadow: 0 0 0 0 rgba(0,196,140,0); } }
        .animate-success-flash { animation: successFlash 1s ease-out; }

        /* Bouncier toast entrance + a shrinking timer bar so the auto-dismiss is visible, not just felt */
...
<body class="text-slate-800 min-h-screen flex flex-col">
    ...
    <!-- LOGIN / REGISTER -->
    <div id="view-login" class="flex-1 flex flex-col justify-center items-center p-4 bg-[#0B1220] relative overflow-hidden">
        <div class="login-blob-wrap" id="login-blob-wrap-a"><div class="login-blob login-blob-a"></div></div>
        <div class="login-blob-wrap" id="login-blob-wrap-b"><div class="login-blob login-blob-b"></div></div>
        <div class="login-spotlight" id="login-spotlight"></div>
        <div class="max-w-md w-full bg-[#0E1626] border border-slate-800 p-8 rounded-2xl shadow-2xl space-y-6 animate-in relative z-10">
            ...
            <form id="signup-form" onsubmit="handleRegister(event)" class="hidden space-y-4">
                <div>
                    ...
                    <input type="text" id="reg-name" required placeholder="user name" class="w-full bg-[#131B2E] border border-slate-800 rounded-xl py-2.5 px-4 text-white placeholder-slate-600 focus:outline-none focus:border-[#00C48C] transition-all">
                </div>
                <div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <button type="button" onclick="togglePasswordVisibility('reg-password', 'reg-eye-icon')" class="absolute inset-y-0 right-3 flex items-center px-2 text-slate-400 hover:text-white transition-all">
                            <i id="reg-eye-icon" class="fa-regular fa-eye-slash text-sm"></i>
                        </button>
                    </div>
                </div>
        ...
        <main class="flex-1 flex flex-col overflow-y-auto">
            <header class="bg-white border-b border-slate-100 py-4 px-6 md:px-8 flex justify-between items-center gap-4 shrink-0">
                ...
                <div class="flex items-center gap-3">
                    <button type="button" onclick="handleThemeToggleClick(event)" id="header-theme-toggle" aria-label="Toggle dark mode" title="Toggle dark / light mode" class="h-9 w-9 shrink-0 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 flex items-center justify-center transition-all">
                        <i class="fa-solid fa-moon text-sm" id="header-theme-icon"></i>
                    </button>
            ...
            <div class="flex-1 p-6 md:p-8 space-y-6 max-w-7xl w-full mx-auto">
                ...
                <div id="tab-dashboard" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4 animate-in" style="animation-delay:0ms">
                        ...
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4 animate-in" style="animation-delay:80ms">
                            <div class="flex justify-between items-center">
                        ...
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4 animate-in" style="animation-delay:160ms">
                            <div class="flex justify-between items-center">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div id="expense-record-card" class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4 transition-all duration-300 animate-in">
                    ...
                    <div id="transactions-section" class="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden transition-all duration-300 animate-in">
                ...
                <div id="tab-analytics" class="hidden space-y-6">
                    <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm animate-in">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div class="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm animate-in" style="animation-delay:0ms">
                        ...
                        <div class="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm animate-in" style="animation-delay:60ms">
                        ...
                        <div class="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm animate-in" style="animation-delay:120ms">
                        ...
                        <div class="bg-white border border-slate-100 p-5 rounded-2xl shadow-sm animate-in" style="animation-delay:180ms">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm space-y-4 animate-in">
                            ...
                            <div class="relative h-64 pb-2 px-2 pt-4">
                                ...
                                <div id="analytics-bar-chart-fallback" class="hidden absolute inset-0 flex items-center justify-center text-center text-xs text-slate-400 px-6"></div>
                            </div>
                        </div>
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm flex flex-col justify-between space-y-4 animate-in" style="animation-delay:80ms">
                            <div>
                ...
                <div id="tab-profile" class="hidden space-y-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm flex flex-col justify-between space-y-6 animate-in" style="animation-delay:0ms">
                        ...
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl lg:col-span-2 shadow-sm animate-in" style="animation-delay:80ms">
                        ...
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl lg:col-span-3 shadow-sm animate-in" style="animation-delay:120ms">
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div class="flex items-center gap-3">
                                    <div class="h-10 w-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-500 shrink-0 text-lg"><i class="fa-solid fa-circle-half-stroke"></i></div>
                                    <div>
                                        <h3 class="font-bold text-base text-slate-800">Appearance</h3>
                                        <p class="text-xs text-slate-400 mt-0.5">Currently using <span id="theme-status-label" class="font-semibold text-slate-600">Light Mode</span> · saved to your account</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3 self-start sm:self-auto bg-slate-50 border border-slate-100 rounded-full pl-3.5 pr-2 py-2">
                                    <i class="fa-solid fa-sun text-amber-400 text-xs"></i>
                                    <button type="button" id="theme-toggle-switch" onclick="handleThemeToggleClick(event)" role="switch" aria-checked="false" aria-label="Toggle dark mode" class="relative w-12 h-7 rounded-full bg-slate-200 shrink-0 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#00C48C]">
                                        <span id="theme-toggle-thumb" class="absolute top-1 left-1 h-5 w-5 rounded-full bg-white shadow-md flex items-center justify-center text-[9px] text-amber-500 translate-x-0">
                        ...
                        <div class="bg-white border border-slate-100 p-6 rounded-2xl lg:col-span-3 shadow-sm space-y-5 animate-in" style="animation-delay:160ms">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                <div class="quote-card bg-gradient-to-br from-[#0B1220] to-[#1E293B] text-white p-6 rounded-2xl relative overflow-hidden flex flex-col justify-between min-h-[220px]">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <div class="tip-card bg-slate-50 border border-slate-100 p-4 rounded-xl">
    ...
    <!-- BUDGET MODAL -->
    <div id="budget-modal" class="hidden fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex justify-center items-center p-4 modal-backdrop-fade">
        <div class="bg-white border border-slate-100 max-w-md w-full rounded-2xl p-6 shadow-2xl space-y-4 modal-panel-pop">
            <div class="flex justify-between items-center border-b border-slate-100 pb-3">
            ...
            <div class="flex justify-end gap-2 pt-2 border-t border-slate-100">
                ...
                <button onclick="handleSetBudget()" id="save-budgets-btn" class="bg-[#00C48C] hover:bg-[#00B07C] text-white text-sm font-bold px-4 py-2 rounded-xl transition-all shadow-md shadow-emerald-500/10">Save Budgets</button>
            </div>
        </div>
    </div>

    <!-- BUDGET EXCEEDED MODAL -->
    <div id="budget-exceeded-modal" class="hidden fixed inset-0 z-[60] bg-slate-950/80 backdrop-blur-sm flex justify-center items-center p-4 modal-backdrop-fade">
        <div class="bg-white border border-slate-100 max-w-sm w-full rounded-2xl p-6 shadow-2xl space-y-4 text-center modal-panel-pop">
