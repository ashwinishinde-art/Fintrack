...
/* ============================================================
   LOGIN SCREEN — cursor-reactive effects
   Moves the ambient blobs slightly with the pointer (parallax),
   tilts the card in 3D toward the cursor, and positions a soft
   spotlight glow under the pointer. Skipped entirely on touch
   devices and when the user prefers reduced motion, and it never
   fights the card's one-time entrance animation because that
   animation class is dropped as soon as it finishes.
   ============================================================ */
function initLoginScreenFX() {
    const loginView = document.getElementById('view-login');
    const card = loginView ? loginView.querySelector('.login-card') : null;
    if (!loginView || !card) return;

    card.addEventListener('animationend', (e) => {
        if (e.animationName === 'fadeInUp') card.classList.remove('animate-in');
    });
    ...
    const blobA = loginView.querySelector('.login-blob-parallax-a');
    const blobB = loginView.querySelector('.login-blob-parallax-b');
    ...
    function applyLoginScreenFX(e) {
        const viewRect = loginView.getBoundingClientRect();
        const vx = (e.clientX - viewRect.left) / viewRect.width - 0.5;
        const vy = (e.clientY - viewRect.top) / viewRect.height - 0.5;

        if (blobA) blobA.style.transform = `translate(${vx * -34}px, ${vy * -34}px)`;
        if (blobB) blobB.style.transform = `translate(${vx * 30}px, ${vy * 30}px)`;

        const cardRect = card.getBoundingClientRect();
        const cx = (e.clientX - cardRect.left) / cardRect.width;
        const cy = (e.clientY - cardRect.top) / cardRect.height;
        const withinCard = cx >= 0 && cx <= 1 && cy >= 0 && cy <= 1;

        if (withinCard) {
            const tiltMax = 3.5; // degrees — kept subtle for a finance app, not gimmicky
            const rotateY = (cx - 0.5) * tiltMax * 2;
            const rotateX = (0.5 - cy) * tiltMax * 2;
            card.style.transform = `perspective(1000px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`;
            card.style.setProperty('--spot-x', `${cx * 100}%`);
            card.style.setProperty('--spot-y', `${cy * 100}%`);
...
function togglePasswordVisibility(inputId, iconId) {
    ...
    if (input.type === 'password') {
        // Currently hidden — reveal it, and show the open eye since
        // the password is now visible on screen.
        input.type = 'text';
        icon.className = 'fa-regular fa-eye text-sm text-white';
    } else {
        // Currently visible — hide it again, and show the slashed eye
        // since the password is no longer on screen.
        input.type = 'password';
