...
function renderAnalyticsCharts() {
    ...
    if (barCanvas) {
        ...
        // Always fully destroy and rebuild the chart instance. Re-using an
        // existing instance after the canvas was previously hidden/resized
        // is what caused the chart to render with invisible/blank bars.
        if (categoryBarChart) {
    ...
    document.getElementById('analysis-reserves-val').innerText = `₹${Math.max(0, remaining).toLocaleString()}`;
    document.getElementById('analysis-consumed-bar').style.width = `${Math.min(spentPercent, 100)}%`;
